...
void main(void)
{
	...
	while (1){
		if(UART_bCmdCheck()) {     					 // Wait for command from PC  
			...
			if(strPtr = UART_szGetParam()) {			// More than delimiter"
				...
				if ((strPtr[0] == 'a') || (strPtr[0] == 'A')){ 		// if receive "a" or "A"
					...
					UART_CPutString("\r NoldPSoClab4 \r\n");
					lcdClean();
					LCD_Position(0,3);				// Place LCD cursor at row 0, col 0
		...
		if( readOneWire == 2 ){
			...
			UART_CPutString("\r\nAnalog: ");
			UART_PutString(itoa(intToString,valueAnalog,10));
			UART_CPutString("\r\nOneWire: ");
			UART_PutString(itoa(intToString,valueOneWire,10));
